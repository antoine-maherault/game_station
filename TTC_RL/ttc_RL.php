...
if($_SESSION["count"]%2 && sizeof($_SESSION["empty"])>=1 && !isset($_SESSION['over'])&& $_SESSION["count"]<8){ // computer
    // add situation BDD si n'existe pas déjà
    $_SESSION['debug'] = $_SESSION["tab"] ;
    if($_SESSION["SQLstop"]==($_SESSION["count"])){
        if($situations==NULL){
            $temp = $_SESSION["tab"];
            $temp = serialize($temp);
            $sql2 ="INSERT Into actions (states) VALUES ('$temp')";
    ...
    while($over == false){
        $box = rand(0, 8);
        if($_SESSION["tab"][$box]=='&nbsp'){ // play random
            // add action BDD through ID 
            $temp = $box;
            $temp = serialize($temp);
            $temp2 = $_SESSION["tab"];
            $temp2 = serialize($temp);
            $sql2 ="UPDATE actions SET act = '$temp' WHERE id = '$id'";
            $query = $conn->query($sql2);  
            // play
            $_SESSION["tab"][$box] = "O";
            $_SESSION["count"] ++;
...
if($_SESSION['over'] == "X" && $_SESSION["SQLstop"] == false){
// add situation BDD
// $temp = [$_SESSION["tab"]];
// $temp = serialize($temp);
// $sql2 ="INSERT Into  actions (board) VALUES ('$temp')";
// $query = $conn->query($sql2);    
// $_SESSION["SQLstop"] = true;
}  

if($_SESSION['over'] == "O"){
// stocker et add BDD dernier coup joué par 0 + reward
$_SESSION['memory'] = [$_SESSION["tab"], 2];

}

if($_SESSION['over'] == "/"){
// stocker et add BDD dernier coup joué par 0 + reward
$_SESSION['memory'] = [$_SESSION["tab"], 0];
}
